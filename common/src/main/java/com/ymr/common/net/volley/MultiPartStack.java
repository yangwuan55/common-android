...
//            }
	...
	private static void setMultiPartBody(HttpEntityEnclosingRequestBase httpRequest,
		...
		Map<File,String> fileUpload = ((MultiPartRequest) request).getFileUploads();
		for (Map.Entry<File,String> entry : fileUpload.entrySet()) {
			builder.addPart(entry.getValue(), new FileBody((File) entry.getKey()));
        }
